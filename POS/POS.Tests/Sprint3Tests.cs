...
public class Sprint3Tests
{
    ...
    [Fact] public async Task ProductionOrder_WithPastDate_IsRejected(){await using var f=await Fixture.Create();var request=NewLot();request.StartDate=DateTime.Today.AddDays(-1);await Assert.ThrowsAsync<BusinessValidationException>(()=>f.Service.CreateAsync(request,1));}
    [Fact] public async Task FutureProductionOrder_CannotStartEarly(){await using var f=await Fixture.Create();var request=NewLot();request.StartDate=DateTime.Today.AddDays(1);var lot=await f.Service.CreateAsync(request,1);await Assert.ThrowsAsync<BusinessValidationException>(()=>f.Service.StartAsync(lot.Id,1));}
    ...
    [Fact] public async Task StartingLot_CalculatesRecipeAutomatically(){await using var f=await Fixture.Create();var lot=await f.CreateLot(8);var x=await f.Service.StartAsync(lot.Id,1);Assert.Equal(10,Assert.Single(x!.Materials).QuantityUsed);f.Db.ChangeTracker.Clear();Assert.Equal(90,(await f.Db.RawMaterials.SingleAsync()).CurrentStock);}
    [Fact] public async Task StartingLot_RejectsInsufficientRecipeStock(){await using var f=await Fixture.Create();var lot=await f.CreateLot(100);await Assert.ThrowsAsync<BusinessValidationException>(()=>f.Service.StartAsync(lot.Id,1));}
    [Fact] public async Task StartingLot_RequiresRecipe(){await using var f=await Fixture.Create();f.Db.ProductRecipeItems.RemoveRange(f.Db.ProductRecipeItems);await f.Db.SaveChangesAsync();var lot=await f.CreateLot();await Assert.ThrowsAsync<BusinessValidationException>(()=>f.Service.StartAsync(lot.Id,1));}
    [Fact] public async Task Consumption_IsLinkedToReceptionAndSupplier(){await using var f=await Fixture.Create();var lot=await f.CreateLot();var x=await f.Service.ConsumeAsync(lot.Id,Consume(10),1);var origin=Assert.Single(Assert.Single(x!.Materials).Origins);Assert.Equal("Proveedor trazable",origin.SupplierName);Assert.Equal(10,origin.Quantity);}
    ...
    private static void AssertModifyRolesExclude(string role){foreach(var name in new[]{nameof(ProductionLotsController.Post),nameof(ProductionLotsController.Consume),nameof(ProductionLotsController.Start),nameof(ProductionLotsController.Finish)}){var method=typeof(ProductionLotsController).GetMethod(name)!;var roles=method.GetCustomAttributes(typeof(AuthorizeAttribute),true).Cast<AuthorizeAttribute>().Single().Roles!;Assert.Contains("Administrador",roles);Assert.Contains("Producción",roles);Assert.DoesNotContain(role,roles);}}
    ...
    private sealed class Fixture:IAsyncDisposable
    {
        ...
        public static async Task<Fixture>Create(bool failing=false){var c=new SqliteConnection("Data Source=:memory:");await c.OpenAsync();var o=new DbContextOptionsBuilder<AppDbContext>().UseSqlite(c).Options;AppDbContext db=failing?new FailingContext(o):new AppDbContext(o);await db.Database.EnsureCreatedAsync();var role=new RoleEntity{Id=1,Name="Producción",IsActive=true};var user=new User{Id=1,Name="Ana",LastName="Paz",Username="produccion",Email="p@test.bo",PasswordHash="hash",Role=role,RoleId=1,IsActive=true};var material=new RawMaterial{Id=1,Name="Palmito fresco",UnitOfMeasure="kg",CurrentStock=100,MinimumStock=10,IsActive=true};var product=new InventoryProduct{Id=1,Name="Palmito entero",UnitOfMeasure="unidad",CurrentStock=10,MinimumStock=5,IsActive=true,Recipe=[new ProductRecipeItem{Id=1,RawMaterialId=1,QuantityPerUnit=1.25m}]};var supplier=new Supplier{Id=1,Name="Proveedor trazable",Nit="123",IsActive=true};var reception=new Reception{Id=1,Supplier=supplier,SupplierId=1,User=user,UserId=1,Date=DateTime.Today.AddDays(-1),Status=ReceptionStatuses.Confirmed,Details=[new ReceptionDetail{Id=1,RawMaterial=material,RawMaterialId=1,Quantity=100}]};db.AddRange(role,user,material,product,supplier,reception);await db.SaveChangesAsync();return new(){Connection=c,Db=db,Service=new(db),User=user,Material=material,Product=product};}
        public Task<ProductionLotResponse>CreateLot(int planned=100){var request=NewLot();request.PlannedQuantity=planned;return Service.CreateAsync(request,User.Id);}
